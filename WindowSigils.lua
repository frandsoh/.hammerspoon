...
function obj:bindHotkeys(mapping)
  if mapping["enter"] then
    local mods, key = table.unpack(mapping["enter"])
    self.mode = hs.hotkey.modal.new(mods, key)
    function self.mode:exited()
      obj:refresh(false)
      hs.alert("WindowSigils exited")
    end
    function self.mode:entered()
      obj:refresh(true)
      hs.alert("WindowSigils entered")
    end
    self.mode:bind(
      {},
      "escape",
...
---@return table
local function map_screen_data()
  local tbl =
    hs.fnutils.map(
...
function obj:start()
  ...
  -- print(hs.inspect(self.window_filter))
  self:refresh()
end
...
function obj:window(sigil)
  if type(sigil) == "number" then
    return self:orderedWindows()[tostring(sigil)]
...
---
--- Parameters:
---@param show_sigils boolean
function obj:refresh(show_sigils)
  show_sigils = show_sigils or false
  for _, screen_data in ipairs(self.screens) do
    ...
    function()
      ...
      screen_data.canvas:show()
    else
      screen_data.canvas:hide()
